...
const { generateAMC } = require("../utils/generateAMC");
const fs = require("fs");

const createAMC = async (req, res) => {
  try {
    const { type, clientName, userID, services, fromDate, toDate } = req.body;

    if (!type || !clientName || !services || !fromDate || !toDate) {
      return res.status(400).json({
        success: false,
        message:
          "type, clientName, userID, service, fromDate, and toDate are required",
      });
    }
    ...
    if (
      !moment(formattedFromDate, "DD/MM/YYYY", true).isValid() ||
      !moment(formattedToDate, "DD/MM/YYYY", true).isValid()
    ) {
      return res.status(400).json({
        success: false,
    ...
    const amcCount = await amcModel.countDocuments(); // Get the count of sales
    const billNo = 1000 + amcCount; // Start bill number from 1000 and increment by the number of sales
    const path = `/public/${type}_receipt_${billNo}.pdf`;
    ...
    const pdfBuffer = await generateAMC(savedAmc);
    fs.writeFileSync("." + path, pdfBuffer);

    const populatedAmc = await amcModel
      .findById(savedAmc._id)
      .populate("clientName")
      .populate("userID")
      .populate("services");
    res.status(200).json({
...
const getAllAMC = async (req, res) => {
  try {
    const amcRecords = await amcModel
      .find()
      .populate("clientName")
      .populate("userID")
      .populate("services");
...
const getAMCById = async (req, res) => {
  try {
    const amcRecord = await amcModel
...
const getAllBYDateAMCFE = async (req, res) => {
  try {
    ...
    if (feid) {
      amcRecords = amcRecords.filter(
        (record) => record.userID._id.toString() === feid
      );
    }
